...
int main() {
    ...
    double appetizerPrices[100], mainCoursePrices[100];
    
    for (int i = 0; i < numAppetizers; i++) {
    cin >> appetizerPrices[i];
    }
    
    for (int i = 0; i < numMainCourses; i++) {
        cin >> mainCoursePrices[i];
    }
    
    Appetizer appetizer;
    MainCourse m;
    appetizer.setPrices(appetizerPrices, numAppetizers);
    m.setPrices(mainCoursePrices, numMainCourses);
    double totalCost = (appetizer.calculatePrice() + m.calculatePrice()) * numPersons;
    cout << "Rs. " << fixed << setprecision(2) << totalCost << endl;
    return 0;
