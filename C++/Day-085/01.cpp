...
class Investment{
    protected:
        ...
        double time_period;
    public:
        Investment(double a, double r, double t) : amt(a), rate(r), time_period(t){}
};

class FixedDeposit : public Investment{
    public:
        FixedDeposit(double a, double r, double t) : Investment(a, r, t){}
        
        double calcMaturityAmt(){
            double c = amt * pow((1 + (rate/100)), time_period);
            return c;
        }
};
...
class SimpleInterest : public FixedDeposit{
    public:
        SimpleInterest(double a, double r, double t) : FixedDeposit(a, r, t){}
        
        void printFD(){
            double c = calcMaturityAmt();
                cout << "Maturity Amount: " << fixed << setprecision(2) << c << endl;
        }
        
        void calcSimpleInterest(){
            double c = amt * rate * time_period / 100;
            cout << "Simple Interest: " << fixed << setprecision(2) << c << endl;
        }
};
