...
class Order{
    protected:
        ...
        double dis;
    public:
        Order(double p, int q, double d): price(p), quant(q), dis(d){}
};

class FinalOrder : public Order{
    public:
        FinalOrder(double p, int q, double d) : Order (p, q, d){}
        
        double TotalCost(){
            double tot = price*quant - price*quant*dis/100;
            cout << "Total Cost: " << fixed << setprecision(2) << tot <<endl;
            return tot;
        }
};
...
class DiscountedOrder : public FinalOrder{
    protected:
        double add_dis;
    public:
        DiscountedOrder(double p, int q, double d, double ad) : FinalOrder(p, q, d), add_dis(ad) {}
        
        void FinalCost(){
            double total = TotalCost();
            double final = total - total*add_dis/100;
            cout << "Final Cost: " << fixed << setprecision(2) << final <<endl;
        }
};
...
int main(){
    double price;
    int quantity;
    double discount, additional_discount;
    
    cin >> price >> quantity >> discount >> additional_discount;
    
    DiscountedOrder d(price, quantity, discount, additional_discount);
