...
class Forest{
    ...
    double width;
    public:
        Forest(double a, double b) : sideLength(a), rainfall(b), length(0), width(0) {}
        
        Forest(double a, double b, double c) : length(a), width(b), rainfall(c), sideLength(0) {}
        
        double sqaureForest(){
            return sideLength*sideLength*rainfall;
        }
        
        double rectangleForest(){
            return length*width*rainfall;
        }
};
...
int main(){
    ...
    Forest f1(a, b);
    cout << fixed << setprecision(2) << f1.sqaureForest() << " mm" << endl;

    Forest f2(c, d, e);
    cout << fixed << setprecision(2) << f2.rectangleForest() << " mm" << endl;
