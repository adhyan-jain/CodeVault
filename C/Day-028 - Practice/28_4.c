...
Input Format:
The first line contains a single integer n, denoting the number of boxes. n lines follow with three integers on each line separated by single spaces — 
lengthi, widthi, heighti which are length, width, and height in feet of the i-th box.
...
Output Format:
For every box from the input which has a height less than 41 feet, print its volume in a separate line.
*/
